...
namespace GridCell
{
    ...
    public class Stage : ScriptableObject
    {
        ...
        private bool IsInIndex(Cell[] cells, int width, int y, int x)
        {
        ...
        public void ResizeCells(int oldHeight, int oldWidth)
        {
            ...
            for (int yIndex = 0; yIndex < _height; yIndex++)
            {
                ...
                {
                    if (old != null &&
                        IsInIndex(old, oldWidth, yIndex, xIndex) &&
                        IsInIndex(_cells, _width, yIndex, xIndex))
                    {
